import { additionalFees, gratuityNote, hourlyRates, mainRates, serviceTypes } from '@/data/pricing'
      ...
      <div className="container">
        ...
        <div className="text-center mb-5">
          ...
          <p className="lead text-muted">
            Elite Transportation Pricing Between Park City and The Salt Lake International Airport
          </p>
        ...
        <div className="card shadow-sm mb-5 border-0">
          <div className="card-header py-3" style={{ backgroundColor: '#1a237e' }}>
                ...
                <tbody>
                  {hourlyRates.map((rate, index) => (
                    <tr key={index}>
                      ...
                      <td>{rate.capacity}</td>
                      <td className="text-success fw-bold">${rate.rate}/hour</td>
                      <td>{rate.duration}</td>
                    </tr>
                  ))}
                </tbody>
                      ...
                      {additionalFees.map((fee, index) => (
                        <tr key={index}>
                          <td>
                            ...
                            {fee.note && <small className="text-muted d-block">{fee.note}</small>}
                          </td>
                          <td className="text-success fw-bold">
                            {fee.fee === 0 ? 'Free (inquire for availability)' : `Add $${fee.fee}`}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        {showModal && selectedRate && (
          <PriceDetailsModal selectedRate={selectedRate} onClose={handleCloseModal} />
        )}
      </div>
    </section>
  )
